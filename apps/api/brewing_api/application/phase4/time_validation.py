...
def _aware(value: datetime) -> datetime:
    if value.tzinfo is None:
        raise DomainError("Timestamp must include an explicit UTC offset", 422, code="TIMESTAMP_NOT_UTC")
...
def validate_observed_at(
    ...
    *,
    ...
    if observed > now + FUTURE_SKEW:
        raise DomainError("observed_at cannot be more than five minutes in the future", 422, code="TIMESTAMP_FUTURE")
    if observed < pitch - FUTURE_SKEW:
        raise DomainError("observed_at cannot precede pitch time", 422, code="TIMESTAMP_BEFORE_PITCH")

    activation_started = stage.current_activation_started_at or stage.first_started_at or stage.started_at
...
def assert_correction_window(session: FermentationSession, *, server_now: datetime | None = None) -> None:
    now = server_now or utc_now()
...
def require_non_paused_for_new_measurement(session: FermentationSession) -> None:
    if session.status == "PAUSED":
        raise ConflictError(
            "Normal measurements cannot be recorded while the session is paused",
            code="SESSION_PAUSED",
        )
