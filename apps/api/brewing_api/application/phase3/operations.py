...
from brewing_api.domain.brew_day.constants import OPERATION_SCHEMA_VERSION
from brewing_api.domain.brew_day.models import BrewOperation
...
def lookup_operation(
    ...
    return db.scalar(
        select(BrewOperation).where(
            BrewOperation.actor_user_id == actor_id,
            BrewOperation.use_case == use_case,
            BrewOperation.aggregate_type == aggregate_type,
            BrewOperation.aggregate_id == aggregate_id,
            BrewOperation.operation_id == operation_id,
        )
    )
...
def store_success(
    ...
    persist_operation(
        ...
        aggregate_id,
        operation_id,
        operation_fingerprint(document),
        http_status,
        result,
        resource_type,
        resource_id,
