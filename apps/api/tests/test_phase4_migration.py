...
EXPECTED_REVISION = "0015_phase4_journal_media_export"
...
def test_phase4_slice11_tables_exist():
    _require_postgres()
    tables = set(inspect(engine).get_table_names())
    missing = [name for name in PHASE4_SLICE11_TABLES if name not in tables]
    assert missing == []


def test_phase4_slice14_tables_exist():
    _require_postgres()
    tables = set(inspect(engine).get_table_names())
    missing = [name for name in PHASE4_SLICE14_TABLES if name not in tables]
    assert missing == []


def test_phase4_slice15_tables_exist():
    _require_postgres()
    tables = set(inspect(engine).get_table_names())
    missing = [name for name in PHASE4_SLICE15_TABLES if name not in tables]
    assert missing == []
